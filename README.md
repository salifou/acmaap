...
Demo of Ansible Automation Platform (AAP) job execution across ACM managed clusters. 

This implementation leverages several key ACM addons and features, including:
- **ManagedServiceAccount addon** for creating service account on managed clusters.
- **ManifestWork**: for setting RBAC on the service accounts.
- **Cluster Proxy addon**: for proxying requests across managed clusters via the Hub cluster.

This work is inspired by the [Multicluster authentication with Ansible Automation Platform](https://developers.redhat.com/articles/2025/09/08/multicluster-authentication-ansible-automation-platform#) article.


## Playbook Summary

1. Connect to the Hub cluster (local-cluster) using a predefined credential.
2. Retrieve the list of managed clusters.
3. Generate a token to connect to each managed cluster using **ManagedServiceAccount**.
4. Set the desired RBAC on each token using **ManifestWork**.
5. Proxy Job exection on managed cluster via the hub cluster (**Cluster Proxy**).
6. Cleanup (delete **ManagedServiceAccount** & **ManifestWork**).
